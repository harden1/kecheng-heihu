...
<script setup name="User">
  ...
  async function queryUser() {
    ...
    await checkUserToBlackLack(form.value.userName).then((res) => {
      console.log('查询用户接口返回数据', res)
      form.value.remark = res.data
      if (res.code === 200 && res.msg === '0') {
        ...
        ElMessageBox.confirm('在MES中没有找到此用户，请联系管理员！', '提示', {
          ...
          console.log('在MES中没有找到此用户，请联系管理员！', mesFlag)
        })
      }
    })
  }
  ...
  async function submitForm() {
    //调用后端接口，拉取黑湖数据，并比对
    console.log('查询用户接口参数', form.value.userName)
    let mesFlag = true
    await checkUserToBlackLack(form.value.userName).then((res) => {
      console.log('查询用户接口返回数据', res)
      form.value.remark = res.data
    ...
    console.log('mesFlag判断有没有找到用户', mesFlag)
    if (mesFlag) {
      console.log('mesFlag判断有没有找到用户')
