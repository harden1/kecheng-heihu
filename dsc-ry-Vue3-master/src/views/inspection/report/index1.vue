...
<script setup name="Report">
  ...
  const data = reactive({
    ...
    rules: {
      summaryId: [{ required: true, message: '主表ID不能为空', trigger: 'blur' }],
      workOrderCode: [{ required: true, message: '工单号不能为空', trigger: 'blur' }],
      reportTime: [{ required: true, message: '报工时间不能为空', trigger: 'blur' }],
      quantity: [{ required: true, message: '报工数量不能为空', trigger: 'blur' }]
    }
  })
  function toScan() {
    router.push({
  ...
  function handleDelete(row) {
    ...
    proxy.$modal
      .confirm('是否确认删除报工记录编号为"' + _ids + '"的数据项？')
      .then(function () {
